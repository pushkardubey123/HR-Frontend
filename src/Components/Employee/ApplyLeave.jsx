 ...
 const onSubmit = async (data) => {
  ...
  return (
    <EmployeeLayout>
      ...
      <h4 className="py-3 d-flex justify-content-center text-warning"><span className="mt-1 mx-2"><VscGitStashApply /></span>Apply Leave</h4>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mb-3">
          <textarea placeholder="Reason..." className="form-control" rows="3" {...register("reason")}></textarea>
          <p className="text-danger">{errors.reason?.message}</p>
        </div>

        <button type="submit" className="btn btn-warning form-control mb-2">Submit Leave</button>
      </form>
    </div> 
    </EmployeeLayout>
  );
